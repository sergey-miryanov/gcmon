...
counter tracks, and a lifetime track per process, which the trace processor
merges into the shared `Processes` row (see
...
UUID, so the same track reuses its UUID across flushes. A `Process` maps
twice, once for its own row and once for the track it draws its lifetime on
([ADR-0011](0011-process-lifetime-and-ordering.md)). Collision-freedom comes
from the counter, not from bit arithmetic.
