# ADR-0015: Draw reconstructed GC loss on a per-interpreter track, one window per poll interval
...
**Loss windows go on a `GC Loss` track of their own, one per
`(process, iid)`.** The track is a custom slice track parented to that
interpreter's group, ranked below the interpreter's pause row inside it
...
**A window is one poll interval**, from the read before the gap to the read
that found it, which is the tightest bound available. A record the previous
poll did not return was either lost already, which that poll reported itself,
or not yet written, so every run a window names finished between those two
reads.
...
**One window, named for the generations that lost records**, `GC Loss(0,2)`. A
...
**A window names the records it is missing**, as `lost_collections` in each
...
window's range for that generation, none twice and none unaccounted for.
...
rebuild the windows from a JSONL capture.
...
- **A loss row whose windows overlap corrupts in silence**, the first END
  closing the wrong window while the trace processor reports
  `misplaced_end_event = 0`. One window per poll cannot produce that shape, so
...
- **One window per generation, each running to that generation's next observed
...
- **Narrowing a window around the records observed inside it**, cutting it
  into pieces that share the counts and pause between them. Rejected: nothing
  in the ring says how the records divide, so every division is a guess and
  the only estimated quantity in the drawing. A share taken by width can hand
  a piece more pause than it is wide. A reader can still narrow it by hand,
  from the `GC Pause` slices on the row above and `observed_count` in the
  args.
- **Clipping a window's far end to the poll's earliest observation anywhere in
...
- **One row for every interpreter.** Rejected: interpreters collect
  concurrently and one poll bounds all of them, so two interpreters' windows
...
- **Retaining intervals and emitting them at shutdown.** Correct, and
  ...
  window in a lump after every GC record. A poll knows both its edges.
