...
| 0066 | **Landed** 2026-09-01 (enhancement) | M | A pid held twice drew one process track, interleaving two processes' pauses under the first one's command line. Each draws its own rows now, `Process 12345` and `Process 12345#2`. [ADR-0028](../docs/adr/0028-draw-every-process-a-row-of-its-own.md) |
| 0067 | **Landed** 2026-09-02 (enhancement) | M | A process's row carried one point and an empty Args panel, and how long gcmon watched it sat on a shared track, drawn clipped. Each row draws a `Lifetime` bar over the observed interval now, saying what gcmon read and missed. [ADR-0028](../docs/adr/0028-draw-every-process-a-row-of-its-own.md) |
...
| 0070 | **Superseded** by 0071 | S | Workers forked in one loop each ran for seconds and were drawn microseconds wide on the `Processes` track. It would have snapped near-equal starts together so the fan-out nested instead of clipping, moving each start by up to a chosen tolerance. 0071 removed the clip the snap worked around, so no start has to move |
