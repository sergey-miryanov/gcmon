...
class SubPhaseRow(PhaseRow, Protocol):
    """A phase inside the pause. `Info` is the protocol its methods take,
    held as a class variable so the fields it names can be read at runtime:
    a nested class does not count as a protocol member to mypy."""
...
# these are present decides which rows a record carries.
_OPTIONAL_FIELDS: Final = attrgetter(
    *sorted(
        set[str]().union(*(get_protocol_members(row.Info) for row in SUB_PHASE_ROWS))
